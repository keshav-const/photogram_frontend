...
//         const result=await axios.post(`${BASE_API_URL}/posts/likeunlike/${id}`,{},
...
//         const result=await axios.post(`${BASE_API_URL}/posts/saveunsavepost/${id}`,{},
//             {withCredentials:true});
//             if(result.data.status=="success"){
...
//     const handelComment=async(id:string)=>{}
...
//     return <div className="mt-20 w-[70%] mx-auto">
//         {/*Main pOst */}
//         {posts.map((post)=>{
...
//                     <p role="button" className="text-sm font-semibold text-blue-700 curosr-pointer"
//                     onClick={()=>{handelComment(post._id)}}>Post</p>
//                 </div>
...
const Feed = () => {
    ...
    //                 toast.success(result.data.message); // Still show success message
    //             }
    //         }
    //     } catch (error) {
        ...
        try {
            ...
            if (user) {
                ...
                const updatedSavedPosts = isSaved
                    ? user.savedPosts.filter((postId) => postId !== id) // Remove post
                    : [...user.savedPosts, id]; // Add post
            ...
            {posts.map((post) => (
                <div key={post._id} className="mt-8">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                        ...
                        user?.savedPosts.includes(post._id) ? "text-red-500" : "text-gray-500"
                    }`}
                        />
